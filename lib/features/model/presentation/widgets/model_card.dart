...
class ModelCard extends ConsumerWidget {
  final ModelEntity item;
  const ModelCard({super.key, required this.item});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final deleting = ref.watch(modelDeleteLoadingProvider).contains(item.id);
    final updating = ref.watch(modelUpdateLoadingProvider).contains(item.id);
  ...
  Future<void> _confirmDelete(BuildContext context, WidgetRef ref, AppLocalizations l10n) async {
    ...
    await ref.read(modelProvider.notifier).delete(
      id: item.id,
    );
  }
}
