...
| payment | object \| null | M | Original payment (paymentType `payment` or default); null if none |
| refundPayments | array | M | Only when transaction has refunds (e.g. status `reversed`). Array of Payment objects with `paymentType` `refund`. Same shape as Payment. |
...
**Response 200** — `data`: transaction detail with full `items` (each with `batches`), `payment` (original payment with `paymentMethods`), and `refundPayments` (array of refund Payment objects, when present). `payment`: null if no payment. `refundPayments`: present when the transaction has refund payments (e.g. when status is `reversed` and refunds were added).
...
`refundPayments` is an array of Payment objects (same shape as `payment`) with `paymentType` `refund`. Empty when the transaction has no refunds.
...
## Endpoints used by the Transaction Detail Page

The transaction detail page (`TransactionDetailPage`) calls the following APIs. All base paths assume auth and branch context.

| Action | Method | Endpoint | When used |
|--------|--------|----------|-----------|
| Load transaction | GET | `/api/transactions/:id` | On mount and when refreshing (e.g. after add payment, refund, reverse). Returns `payment`, `refundPayments`, `items`. |
| Reverse transaction | POST | `/api/transactions/:transactionId/reverse` | When user confirms reverse (body: `{ data: { notes } }`). |
| Create refund payment | POST | `/api/payments/refund` | When user adds a refund on a **reversed** transaction. Body: `{ data: { source: 'transaction', transactionId, originalPaymentId, paymentMethods: [...] } }`. Same request/response shape as payment method creation where applicable. |
| Add payment methods | POST | `/api/payments/:paymentId/methods` | When user adds payment methods to the **original** payment (completed transaction). Body: `{ data: [ { method, amount, attachment?, referenceNumber?, bankId? } ] }`. |
| Update payment method | PUT | `/api/payments/:paymentId/methods/:id` | When user edits a payment method (original or refund). Body: `{ data: { method?, amount?, attachment?, referenceNumber?, bankId? } }`. |

No other endpoints (e.g. GET/DELETE payment methods, GET payment by id) are used on the detail page for the flows above.
...
- **Refund (transaction flow):** From the transaction detail page, when status is `reversed`, "Add refund" creates a new Payment with `paymentType` `refund` and `originalPaymentId` set to the original payment id. Refunds are returned in `refundPayments` and shown in a dedicated section (not mixed with the original payment).
